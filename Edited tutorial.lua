...
function scene:create( event )
    ...
    local function scene2()
        ...
        local function findPattern()
            ...
            pattern[i].number = currentDot
            print(pattern[i].number)
        ...
        local okbg = display.newImage( sceneGroup, "images/smallTealButton.png", system.ResourceDirectory, globals.centerX, 375)
        local okText = display.newText( sceneGroup, "memorized it.", globals.centerX, 375, globals.font.regular, 20 )
        local flashAgainBg = display.newImage( sceneGroup, "images/smallPinkButton.png", system.ResourceDirectory, globals.centerX, 430)
        local flashAgainText = display.newText( sceneGroup, "do that again.", globals.centerX, 430, globals.font.regular, 20 )
        local function gotoScene3()
            if timesFound == 4 then
                ...
                transition.to(flashAgainBg, {time = 750, x = display.contentWidth - 1000})
                transition.to(flashAgainText, {time = 750, x = display.contentWidth - 1000})
                transition.to(memorizeText, {time = 750, x = display.contentWidth - 1000, onComplete = scene3})
                okbg, okText, flashAgainBg, flashAgainText, memorizeText = nil, nil, nil, nil, nil
            end
        end
        local addRemoveListeners
        local function flashAgain()
            addRemoveListeners("remove")
            timesFound = 0
            local function flashDots()
                local i = timesFound
                audio.play(ding[pattern[i].number])
                local function removeFlash()
                    transition.to(pattern[i], {time = 250, xScale = 1, yScale = 1})
                    if i == 4 then
            ...
            timer.performWithDelay( 750, flashDots, 4 )
        end
        
        addRemoveListeners = function(addRemove)
            if addRemove == "add" then
                okbg:addEventListener("tap", gotoScene3)
                flashAgainBg:addEventListener("tap", flashAgain)
            elseif addRemove == "remove" then
                okbg:removeEventListener("tap", gotoScene3)
                flashAgainBg:removeEventListener("tap", flashAgain)
            end
        end
        addRemoveListeners("add")
    end
